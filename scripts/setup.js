...
const FLOW_ORDER = ['flow-handoff', 'flow-agent', 'flow-webhook', 'flow-reminder', 'flow-recovery'];
...
function credentialPayloads(env, names) {
  ...
  return [
    { key: 'postgres', type: 'postgres', name: names.postgres, data: {
      host: db.hostname, port: Number(db.port || 5432), database: db.pathname.replace(/^\//, '') || 'postgres',
...
function buildFlow(name, env, ctx) {
  const raw = readFileSync(join(ROOT, 'blueprints', 'flows', `${name}.json`), 'utf-8');
  ...
  const scalarMap = {
    ...
    // Prompt: se inyectan como literales JS válidos.
  ...
  // Credenciales por id real
  for (const node of flow.nodes || []) {
    if (!node.credentials) continue;
    for (const [ctype, ref] of Object.entries(node.credentials)) {
      // httpHeaderAuth se usa para WhatsApp y para el modelo: distinguimos por el name original.
  ...
  const sobran = [...new Set(JSON.stringify(flow).match(/__[A-Z0-9_]+__/g) || [])];
  if (sobran.length) throw new Error(`${name}: marcadores sin resolver → ${sobran.join(', ')}`);

  return { name: flow.name, nodes: flow.nodes, connections: flow.connections, settings: { executionOrder: 'v1' } };
}
